...
export default function App() {
  ...
  // Rendered unconditionally, above every other state — this is the one
  // thing that must show even if sign-in itself is broken, since it's the
  // way out of exactly that kind of bug.
  return (
  ...
  function renderBody() {
    if (!firebaseConfigured) {
      return (
        <div className="flex h-full w-full items-center justify-center bg-app-light px-6">
    ...
    return (
      <AnimatePresence mode="wait">
        {!user ? (
          <motion.div key="login" exit={{ opacity: 0 }} transition={{ duration: 0.25 }} className="h-full w-full">
